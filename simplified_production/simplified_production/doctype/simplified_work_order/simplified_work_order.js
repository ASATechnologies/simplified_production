...
frappe.ui.form.on("Simplified Work Order", {
  ...
  setup: function (frm) {
    frm.fields_dict["consumed_by_manufacturing"].grid.get_field(
      "source_warehouse"
    ).get_query = function (dn, c, b) {
      return {
        filters: {
          company: dn.company,
        },
    ...
    frm.fields_dict["produced_from_manufacturing"].grid.get_field(
      "cost_center"
    ).get_query = function (dn, c, b) {
      return {
        filters: {
...
function calc_apportion(frm) {
  ...
  items.map((cur) => {
    const total_cost =
      ((cur.cost * cur.qty) / get_total_cost_of_production) *
      total_incoming_cost;
